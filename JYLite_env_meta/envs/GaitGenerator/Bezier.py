...
import numpy as np
from JYLite_env_meta.robots.LieAlgebra import TransToRp
import copy
...
class BezierGait():
    ...
    def BezierSwing(self, phase, L, LateralFraction, clearance_height=0.1):
    ...
    def GenerateTrajectoryX(self,
        ...
        p_bf_ = copy.deepcopy(T_bf_)
        for i, (key, pb_f) in enumerate(T_bf_.items()):
            # TODO: MAKE THIS MORE ELEGANT
            if key == "FL":
                self.ref_idx = i
                self.dSref[i] = 0.0
            ...
            if Tstance > 0.0:
                step_coord = self.GetFootStep(L, LateralFraction, YawRate,
                                              clearance_height,
                                              penetration_depth, Tstance, pb_f,
                                              i, key)
            else:
                step_coord = np.array([0.0, 0.0, 0.0])
            p_bf_[key][0] = pb_f[0] + step_coord[0]
            p_bf_[key][1] = pb_f[1] + step_coord[1]
            p_bf_[key][2] = pb_f[2] + step_coord[2]
        return p_bf_

    def GenerateTrajectoryX_FR(self,
                           L,
                           LateralFraction,
                           YawRate,
                           vel,
        ...
        # Check contacts
        # print(contacts)
        if contacts[1] == 1 and Tstance > dt:
            self.TD = True
