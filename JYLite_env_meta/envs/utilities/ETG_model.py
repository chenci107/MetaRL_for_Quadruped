...
base_foot = np.array([0.1745,0.164,-0.23,        # foot position related to base (L_hip/2, W_hip/2+L1, -h)
                      0.1745,-0.164,-0.23,
                      -0.1745,0.164,-0.23,
                      -0.1745,-0.164,-0.23,])
...
class ETG_layer():
    def __init__(self,T,dt,H,sigma_sq,phase,amp,T2_radio):
        ...
        for h in range(H):
            ...
            self.u.append(self.forward(t_now))  # e.q.(9)
    ...
    def forward(self,t):
        ...
        for i in range(self.phase.shape[0]):
            x.append(self.amp*np.sin(self.phase[i]+t*self.omega))  # e.q.(7)
...
class ETG_model():
    def __init__(self,task_mode="normal",act_mode="traj",step_y=0.5):
        ...
        self.pose_ori=np.array([0,-0.6283,1.3614] * 4)
    ...
    def act_clip(self,new_act,env):
        if self.act_mode == "pose":
            ...
            for i in range(4):
                ...
                while(1):
                    # perform IK
                    index,angle = env.ComputeMotorAnglesFromFootLocalPosition(leg_id=i,foot_local_position=delta+base_foot[i*3:(i+1)*3])
