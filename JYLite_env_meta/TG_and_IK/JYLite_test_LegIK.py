...
if __name__ == '__main__':
    ...
    while True:
        ...
        '''Method 2: The method copied from A1 (change +-)'''
        ...
        #
        # foot_position_HL = np.array([0.015731, 0.090814, -0.229485])
        # joints_HL = foot_position_in_hip_frame_to_joint_angle(foot_position_HL,l_hip_sign=1)
        #
        # foot_position_HR = np.array([0.015731, -0.090186, -0.229485])
        # joints_HR = foot_position_in_hip_frame_to_joint_angle(foot_position_HR,l_hip_sign=-1)
        #
        # all_joints = np.concatenate((joints_FL,joints_FR,joints_HL,joints_HR),axis=0)
        # ob,reward,done,info = env.step(all_joints)
        '''Method 2: The method copied from A1 (change +-)'''
        '''FL'''
        # xyz_coord = np.array([0.0,0.102,-0.2617])
        # joints = foot_position_in_hip_frame_to_joint_angle(xyz_coord,l_hip_sign=1)
        # all_joints = leg_test_2(joints,leg_idx=0)
        # ob,reward,done,info = env.step(all_joints)
        '''FR'''
        # xyz_coord = np.array([0.0, -0.102, -0.2617])
        # joints = foot_position_in_hip_frame_to_joint_angle(xyz_coord, l_hip_sign=-1)
        # all_joints = leg_test_2(joints, leg_idx=1)
        # ob, reward, done, info = env.step(all_joints)
        '''HL'''
        # xyz_coord = np.array([0.0, 0.102, -0.2617])
        # joints = foot_position_in_hip_frame_to_joint_angle(xyz_coord, l_hip_sign=1)
        # all_joints = leg_test_2(joints, leg_idx=2)
        # ob, reward, done, info = env.step(all_joints)
        '''HR'''
        # xyz_coord = np.array([0.0, -0.102, -0.2617])
        # joints = foot_position_in_hip_frame_to_joint_angle(xyz_coord, l_hip_sign=-1)
        # all_joints = leg_test_2(joints, leg_idx=3)
        # ob, reward, done, info = env.step(all_joints)
        '''--------------------- TEST Inverse IK ------------------------'''
