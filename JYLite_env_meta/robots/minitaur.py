...
import re
import numpy as np
from JYLite_env_meta.robots import minitaur_constants
from JYLite_env_meta.robots import minitaur_motor
from JYLite_env_meta.robots import robot_config
from JYLite_env_meta.robots import action_filter
from JYLite_env_meta.robots import kinematics
...
# 0: GetMotorAngles()
# 1: GetMotorVelocities()
# 2: GetMotorTorques()
# 3: GetRollPitchYaw()
# 4: GetRollPitchYawRate()

'''---------- INERTIA FOR  JYLite----------'''
BASE_LINK_INERTIA = (0.016982120,0.030466501,0.042609956)
LEG_HIP_LINK_INERTIA = (0.00014538,0.00024024,0.00013038)
LEG_THIGH_LINK_INERTIA = (0.001,0.00116,2.68E-04)
LEG_SHANK_LINK_INERTIA = (6.68E-04,6.86E-04,3.155E-05)
IF_USE_MANUAL_INERTIA = True
''''----------------------------------------'''
...
class Minitaur(object):
    ...
    def Step(self, action, control_mode=None):
        ...
        # print('t:',torques)
        torques = np.asarray(torques).reshape(-1, 12)
    ...
    def _RecordInertiaInfoFromURDF(self):
        '''value from pybullet API,different from URDF'''
        if not IF_USE_MANUAL_INERTIA:
            self._base_inertia_urdf = []
            for chassis_id in self._chassis_link_ids:
                self._base_inertia_urdf.append(
                    self._pybullet_client.getDynamicsInfo(self.quadruped,chassis_id)[2])  # 2
            self._leg_inertia_urdf = []
            for leg_id in self._leg_link_ids:
                self._leg_inertia_urdf.append(
                    self._pybullet_client.getDynamicsInfo(self.quadruped,leg_id)[2])   # 2
        else:
            '''value from manual record'''
            self._base_inertia_urdf = [BASE_LINK_INERTIA]
            self._leg_inertia_urdf = [LEG_HIP_LINK_INERTIA,LEG_THIGH_LINK_INERTIA,LEG_SHANK_LINK_INERTIA,
                                      LEG_HIP_LINK_INERTIA, LEG_THIGH_LINK_INERTIA, LEG_SHANK_LINK_INERTIA,
                                      LEG_HIP_LINK_INERTIA, LEG_THIGH_LINK_INERTIA, LEG_SHANK_LINK_INERTIA,
                                      LEG_HIP_LINK_INERTIA, LEG_THIGH_LINK_INERTIA, LEG_SHANK_LINK_INERTIA,]
    ...
    '''more than the JYLite class'''
    def Reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0, default_pose=None, yaw=0.0):
        ...
        """
        defaultpose = default_pose if default_pose is not None else self._GetDefaultInitPosition()
        if reload_urdf:
            self._LoadRobotURDF()
            if self._on_rack:
            ...
            # orientation = [0,0,0.479,0.878]
            orientation = [0, 0, np.sin(yaw / 2.0), np.cos(yaw / 2.0)]
            self._pybullet_client.resetBasePositionAndOrientation(
                self.quadruped, defaultpose,
                orientation)
            # print("origin_oritation:",self._GetDefaultInitOrientation())
    ...
    def _SettleDownForReset(self, default_motor_angles, reset_time):
        ...
        self.ReceiveObservation()
        for _ in range(100):
            self._StepInternal([math.pi / 2] * self.num_motors,motor_control_mode=robot_config.MotorControlMode.POSITION)
    ...
    '''more than JYLite, used by robot_sensors.py'''
    ...
    '''different implement from JYLite'''
    ...
    def GetMotorAngles(self):
        ...
        """
        motor_angles = self._AddSensorNoise(np.array(self._control_observation[0:self.num_motors]),self._observation_noise_stdev[0])
    ...
    def GetMotorVelocities(self):
        ...
        """
        return self._AddSensorNoise(np.array(self._control_observation[self.num_motors:2 * self.num_motors]),
    ...
    def ApplyAction(self, motor_commands, motor_control_mode):
        ...
        # HipX:0, HipY:-0.6283, Knee:1.3614
        # motor_commands[1] = -0.6283
        # time.sleep(1/120)
    ...
    '''more than JYLite'''
    def SetFootSpinFriction(self, foot_friction):
        """Set the lateral friction of the feet.
        ...
        for link_id in self._foot_link_ids:
            self._pybullet_client.changeDynamics(self.quadruped,
                                                 link_id,
                                                 spinningFriction=foot_friction)

    '''more than JYLite'''
    def GetFootStatus(self):
        """Get the lateral friction of the feet.
        ...
        status_all = []
        for link_id in self._foot_link_ids:
            status = self._pybullet_client.getDynamicsInfo(self.quadruped,
                                                           link_id)
    ...
    '''more than JYLite, used by JYLite'''
